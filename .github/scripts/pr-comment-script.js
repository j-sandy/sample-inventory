...
async function createPRComment(github, context, secrets) {
  ...
  if (fs.existsSync(findingsPath)) {
    try {
      ...
      commentParts.push('');
      commentParts.push('🌐 [Explore SBOM in SSD Portal](' + portalLink + ')');
      commentParts.push('');
      commentParts.push('---');
  ...
  if (fs.existsSync(findingsPath)) {
    ...
    try {
      ...
      try {
        ...
        console.log('');
        console.log('📦 SBOM Metadata:');
        console.log('   • Artifact Name: ' + (metadata.artifactName || 'N/A'));
        console.log('   • Artifact Tag: ' + (metadata.artifactTag || 'N/A'));
        console.log('   • Artifact SHA: ' + (metadata.artifactSha || 'N/A'));
        console.log('   • SBOM Tool: ' + (metadata.sbomTool || 'N/A'));
        console.log('   • Highest Severity: ' + (summary.highestSeverity || 'N/A'));
        console.log('');
        ...
        if (metadata.artifactSha && metadata.artifactName && metadata.artifactTag && metadata.sbomTool) {
          console.log('');
          console.log('🌐 Dynamic Portal Link:');
          const riskStatus = encodeURIComponent(summary.highestSeverity || 'Low Risk');
          const imageId = encodeURIComponent(metadata.artifactName);
          const artifactSha = metadata.artifactSha; // Don't encode the colon
          const versionNo = encodeURIComponent(metadata.artifactTag);
